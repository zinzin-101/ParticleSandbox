    ...
    LPSTR lpCmdLine, int nCmdShow)
{
    ...
    sf::Text text_fps;
    text_fps.setFont(font); // font is a sf::Font
    text_fps.setCharacterSize(64); // in pixels, not points!
    ...
    const uint32_t     max_objects_count     = 3500;
    ...
    while (Msg.message != WM_QUIT)
    {
        if (PeekMessage(&Msg, NULL, 0, 0, PM_REMOVE))
        {
            ...
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::G) && (Gdown == false)) {
                enableGravity = !enableGravity;
                Gdown = true;
            }
            if (!sf::Keyboard::isKeyPressed(sf::Keyboard::G) && Gdown) { Gdown = false; }

            // measure fps
            sf::Time elapsed1 = clk_fps.getElapsedTime();
            clk_fps.restart();
