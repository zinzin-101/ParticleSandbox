...
int32_t main(int32_t, char*[])
{
    ...
    const uint32_t     max_objects_count     = 40;
    ...
    for (int i = 1; i < 40; i++) {
        ...
        //std::cout << link1.obj_1.position.x << " " << link1.obj_2.position.x << std::endl;
        objIndex++;
    }
    startRopePos = { 700,500 };
    solver.addObject(startRopePos, 5, true);
    solver.setObjectVelocity(solver.m_objects[objIndex], sf::Vector2f{ 0,0 });
    solver.m_objects[objIndex].color = getPinkBlue((objIndex-40) / 40.0f);
    objIndex++;
    for (int i = 1; i < 40; i++) {
        solver.addObject(startRopePos - sf::Vector2f(i * 15, 0), 5, false);
        solver.setObjectVelocity(solver.m_objects[objIndex], sf::Vector2f{ 0,0 });
